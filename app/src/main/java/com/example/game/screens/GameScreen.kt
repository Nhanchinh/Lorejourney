...
class GameScreen(
    ...
    private val controlPressedPaint =
            Paint().apply {
                isAntiAlias = true
    ...
    private val controlBorderPaint =
            Paint().apply {
                isAntiAlias = true
                color = Color.WHITE
    ...
    private fun initLevel() {
        ...
        if (levelId == 6) {
            println(
                    "Map6 (Sprite-based) loaded - Width: ${gameMap.width}, Height: ${gameMap.height}, SpawnX: ${gameMap.playerSpawnX}, SpawnY: ${gameMap.playerSpawnY}"
            )
        } else {
            println(
                    "Map loaded - Width: ${gameMap.width}, Height: ${gameMap.height}, SpawnX: ${gameMap.playerSpawnX}, SpawnY: ${gameMap.playerSpawnY}"
            )
        }
    ...
    private fun drawUI(canvas: Canvas) {
        ...
        if (levelId == 3) {
            ...
            shadowMechanic?.getAllShadowsInfo()?.forEachIndexed { index, info ->
                canvas.drawText(
                        "Shadow ${index + 1}: ${info.directionChangeCount}/4 turns",
                        50f,
                        190f + (index * 80f),
                        infoPaint
                )
                canvas.drawText(
                        "Following: ${info.isFollowing}",
                        50f,
                        220f + (index * 80f),
                        infoPaint
                )
                ...
                if (helpButton.contains(x, y)) {
                    ...
                    val videoName =
                            when (levelId) {
                                ...
                                3, 4 -> "mechanic3" // Shadow mechanics
                                5, 6, 7 -> "mechanic4" // Advanced puzzles
                                8 -> "mechanic5" // More advanced
                                9, 10, 11, 12, 13, 14, 15 -> "mechanic6" // Final mechanics
            ...
            storyEntry?.let { story ->
                storyDialog.show(story.segments) {
                    ...
                    val videoName =
                            when (levelId) {
                                1 -> "mechanic1" // Basic controls
                                5 -> "mechanic4" // Advanced puzzles
                                8 -> "mechanic5" // More advanced
                                9 -> "mechanic6" // Final mechanics
                                else -> null // Default
                            }
                    if (videoName != null) {
                        containerLayout.post { videoPopup.show(videoName, containerLayout) }
                    }
                }
            }
        }
